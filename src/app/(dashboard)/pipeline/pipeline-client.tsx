...
interface PipelineCard {
  ...
  value: number | null; due_date: string | null; patient_id: string | null; patients: unknown
}
...
export function PipelineClient({ stages, initialCards, patients }: PipelineClientProps) {
  ...
  function openModal(stageId?: string) { setDefaultStageId(stageId); setModalOpen(true) }
  ...
  async function handleMove(cardId: string, newStageId: string) {
    ...
    setMovingId(null)
    if (result?.error) { toast.error(result.error); setCards(initialCards) }
  }
    ...
    <>
      <div className="flex flex-col gap-6 p-6 h-full">
        <div className="flex items-center justify-between">
          ...
          <Button onClick={() => openModal()} className="bg-blue-600 hover:bg-blue-700 gap-2 shadow-sm">
              ...
              <div key={stage.id} className="flex flex-col gap-3 w-72 flex-shrink-0">
                <div className="flex items-center justify-between px-1">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge variant="outline" className="border-gray-200 text-gray-400 text-xs h-5 px-1.5 bg-white">
                      {stageCards.length}
                    </Badge>
                  </div>
                  {stageValue > 0 && (
                ...
                <div className="flex flex-col gap-2 rounded-xl bg-gray-100/60 border border-gray-200 p-2 min-h-[400px]">
                  {stageCards.map((card) => {
                    ...
                    return (
                      <Card key={card.id}
                        className={`border-gray-200 bg-white hover:shadow-md transition-all cursor-pointer group shadow-sm ${movingId === card.id ? 'opacity-50' : ''}`}>
                        <CardHeader className="pb-2 pt-3 px-3">
                          <div className="flex items-start justify-between gap-2">
                            <p className="text-sm font-medium text-gray-900 leading-snug">{card.title}</p>
                            {nextStage && (
                              <button onClick={() => handleMove(card.id, nextStage.id)} disabled={movingId === card.id}
                                title={`Mover para ${nextStage.name}`}
                                className="opacity-0 group-hover:opacity-100 flex-shrink-0 text-gray-300 hover:text-blue-500 transition-all">
                        ...
                        <CardContent className="px-3 pb-3 space-y-1.5">
                          {patientName && (
                            <div className="flex items-center gap-1.5">
                              <div className="h-5 w-5 rounded-full bg-blue-50 flex items-center justify-center text-blue-600 text-[10px] font-bold flex-shrink-0">
                                {patientName[0].toUpperCase()}
                              </div>
                              <span className="text-xs text-gray-500 truncate">{patientName}</span>
                            </div>
                          )}
                          ...
                          {card.due_date && (
                            <p className={`text-[11px] font-medium ${isOverdue ? 'text-red-500' : 'text-gray-400'}`}>
                  ...
                  <Button variant="ghost" onClick={() => openModal(stage.id)}
                    className="w-full text-gray-400 hover:text-gray-600 hover:bg-gray-200/60 gap-2 mt-auto text-xs h-8 border border-dashed border-gray-200">
      ...
      <LeadModal open={modalOpen} onClose={() => setModalOpen(false)}
        stages={stages} patients={patients} defaultStageId={defaultStageId} onCreated={handleCreated} />
    </>
  )
}
